...
const ShareImpactCard = ({ treesPlanted, totalDonationsUsd, totalSwaps, co2Absorbed }: ShareImpactCardProps) => {
  ...
  const handleShareSocial = useCallback(async (platform: string) => {
    setGenerating(true);
    try {
      const canvas = await captureCard();
      if (!canvas) return;
      ...
      if (blob && navigator.share && navigator.canShare) {
        ...
        const shareData = { text: SHARE_TEXT(treesPlanted), url: SHARE_URL, files: [file] };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-background" onClick={() => setOpen(false)}>
      <div className="w-full max-w-xs mx-auto px-4 py-4 flex flex-col h-dvh max-h-dvh" onClick={(e) => e.stopPropagation()}>
        {/* Top bar */}
        ...
        {/* The card — takes remaining space */}
        <div
          ref={cardRef}
        ...
        >
          ...
          >
            <div className="absolute top-0 right-0 w-40 h-40 rounded-full blur-3xl opacity-30" style={{ background: "radial-gradient(circle, #22c55e 0%, transparent 70%)" }} />
            <div className="absolute bottom-0 left-0 w-32 h-32 rounded-full blur-3xl opacity-20" style={{ background: "radial-gradient(circle, #10b981 0%, transparent 70%)" }} />
        ...
        {/* Social share + download — pinned at bottom */}
        <div className="shrink-0 pt-3 space-y-3">
          <div className="flex gap-3 justify-center">
            <button onClick={() => handleShareSocial("x")} disabled={generating} className="w-11 h-11 rounded-xl bg-card border border-border flex items-center justify-center text-foreground hover:bg-muted transition-colors disabled:opacity-50" title="X">
              <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </button>
            <button onClick={() => handleShareSocial("telegram")} disabled={generating} className="w-11 h-11 rounded-xl bg-card border border-border flex items-center justify-center text-foreground hover:bg-muted transition-colors disabled:opacity-50" title="Telegram">
            ...
            <button onClick={() => handleShareSocial("discord")} disabled={generating} className="w-11 h-11 rounded-xl bg-card border border-border flex items-center justify-center text-foreground hover:bg-muted transition-colors disabled:opacity-50" title="Discord">
            ...
            <button onClick={() => handleShareSocial("instagram")} disabled={generating} className="w-11 h-11 rounded-xl bg-card border border-border flex items-center justify-center text-foreground hover:bg-muted transition-colors disabled:opacity-50" title="Instagram">
          ...
          <Button onClick={handleDownload} disabled={generating} className="w-full gap-2" variant="outline">
