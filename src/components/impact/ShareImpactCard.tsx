...
const ShareImpactCard = ({ treesPlanted, totalDonationsUsd, totalSwaps, co2Absorbed }: ShareImpactCardProps) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex flex-col items-center bg-background overflow-y-auto" onClick={() => setOpen(false)}>
      <div className="w-full max-w-xs mx-auto px-4 py-6 space-y-4" onClick={(e) => e.stopPropagation()}>
        {/* Top bar */}
        ...
        {/* Social share buttons — on top */}
        <div className="flex gap-3 justify-center">
          <button
            onClick={() => handleShareSocial("x")}
            disabled={generating}
            className="w-12 h-12 rounded-xl bg-card border border-border flex items-center justify-center text-foreground hover:bg-muted transition-colors disabled:opacity-50"
            title="X"
          >
