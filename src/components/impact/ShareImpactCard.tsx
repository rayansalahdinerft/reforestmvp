...
const SHARE_TEXT = (trees: number) =>
  `I planted ${trees.toLocaleString()} trees with ReforestWallet! 🌱 Join the green DeFi movement`;
  ...
  return (
    ...
    >
      <div className="relative max-w-sm w-full space-y-4" onClick={(e) => e.stopPropagation()}>
        {/* Close */}
        <button
          onClick={() => { setOpen(false); setShowSocials(false); }}
          className="absolute -top-3 -right-3 z-10 w-9 h-9 rounded-full bg-card border border-border flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors shadow-lg"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div
          ...
          style={{
            background: "linear-gradient(145deg, hsl(150 40% 6%) 0%, hsl(150 40% 10%) 30%, hsl(150 35% 12%) 60%, hsl(150 40% 6%) 100%)",
            padding: "2px",
          }}
        >
          <div
            ...
            style={{
              background: "linear-gradient(160deg, hsl(150 30% 8%) 0%, hsl(150 35% 11%) 40%, hsl(150 40% 6%) 100%)",
            }}
          >
            {/* Decorative glows */}
            <div
            ...
            {/* Header - leaf icon only */}
            <div className="flex items-center justify-between mb-6 relative z-10">
              <div className="flex items-center gap-2">
                <img src={leafIcon} alt="" className="w-8 h-8 rounded-lg" />
                <div>
                  <p className="text-foreground text-sm font-bold tracking-tight">ReforestWallet</p>
                  <p className="text-primary/70 text-[10px]">Proof of Impact</p>
                </div>
              </div>
              <div className="px-3 py-1 rounded-full border border-primary/30 bg-primary/10">
                <span className="text-primary text-xs font-bold">Level {currentLvl}</span>
              </div>
            </div>
            ...
            <div className="text-center mb-6 relative z-10">
              ...
              <p className="text-primary/80 text-sm font-medium mt-1">trees planted</p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-4 relative z-10">
              <div className="text-center p-3 rounded-xl bg-muted/10">
                <p className="text-foreground text-lg font-bold tabular-nums">
                  ${totalDonationsUsd.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                </p>
                <p className="text-muted-foreground text-[10px]">Donated</p>
              </div>
              <div className="text-center p-3 rounded-xl bg-muted/10">
                <p className="text-foreground text-lg font-bold tabular-nums">{totalSwaps.toLocaleString()}</p>
                <p className="text-muted-foreground text-[10px]">Swaps</p>
              </div>
              <div className="text-center p-3 rounded-xl bg-muted/10">
                <p className="text-foreground text-lg font-bold tabular-nums">{Math.floor(co2Absorbed).toLocaleString()} kg</p>
                <p className="text-muted-foreground text-[10px]">CO₂/year</p>
              </div>
            </div>

            {/* Footer */}
            <div className="text-center relative z-10 pt-2 border-t border-primary/10">
              <p className="text-primary text-xs font-semibold">reforestmvp.lovable.app</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 justify-center">
          <button
            ...
            className="w-11 h-11 rounded-xl bg-card border border-border flex items-center justify-center text-foreground hover:bg-muted transition-colors"
            title="Share on X"
          >
